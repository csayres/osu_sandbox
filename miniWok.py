...
from parseDesignRef import osuMini as assignmentTable

########### PARAMETERS ####################
SEED = None                 # integer to set the random seed for paths
CONTINUOUS = False           # robots will go on continuous paths forever until the program is killed
UNWIND_ONLY = False         # robots will just unwind to (0,180) and the program will end
DO_COMPLICATED = False      # non-complicated paths will be skipped.  complicated paths are close encounters.
SPEED = 3                   # RPM at output (dont go over 3?)
COLLISION_BUFFER = 2.5      # effective *radius* of beta arm in mm effective beta arm width is 2*collisionBuffer (wouldn't recommend < 2.2)
COLLISION_SHRINK = 0.04     # amount to decrease collisionBuffer by when checking smoothed and simplified paths
################################################
...
def gridInit(seed):
    hasApogee = True
    rg = RobotGrid(angStep, COLLISION_BUFFER, epsilon, seed)
...
def generatePath(rg, plot=False, movie=False, fileIndex=0):
    ...
    rg.simplifyPaths()
    rg.setCollisionBuffer(COLLISION_BUFFER - COLLISION_SHRINK)
    rg.verifySmoothed()

    if rg.smoothCollisions:
        print("smoothing failed with %i collisions, try increasing the collisionBuffer and COLLISION_SHRINK parameters", rg.smoothCollisions)
...
async def runPathPair(fps, seed=0, doComplicated=False, alphaLimit=None):
    ...
    print("otachi, seed=%i, collisionBuffer=%.4f"%(seed, COLLISION_BUFFER))
    try:
...
if __name__ == "__main__":
    ...
    asyncio.run(main(SEED, doCentroid, CONTINUOUS, UNWIND_ONLY, DO_COMPLICATED, alphaLimit))
